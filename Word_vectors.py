"""
Created on Fri May 11 14:04:13 2018
...
def remove_stopwords(product_name, rem_punc=True, otherlist=None):
    """
    Function to remove stopwords from product names. By default will remove
    stopwords from sklearn standard list but extra stopwords can be appended.

    Args:
        product_name (Series): series containg the product names
                               to be vectorized
        rem_punc (bool): it true punctuation is removed from product names,
                         Default is True
        otherlist (list(str)): if exists extra stop words are appended to the
                               list from sklearn.
    Return:
    ...
    # optionally add aditional stopwords
    if otherlist:
        stop = stop.union(set(otherlist))
    # remove stopwords from product names
    product_name = (product_name.apply(lambda x: ' '.join(
...
class LabeledLineSentence(object):
    """
    Class to create tagged document structute for doc2vec.
    ...
    def __iter__(self):
        ...
        for idx, doc in enumerate(self.doc_list):
            yield TaggedDocument(words=doc.split(),
                                 tags=[str(self.labels_list[idx])])


def fit_Doc2vec(product_name, print_sim=True, vector_size=10, window=12,
                min_count=1, workers=4, alpha=0.025,
                min_alpha=0.001, epochs=100):
    """
    ...
    Args:
        ...
        print_sim (bool): If true vector similarities prinited to the screen
        ...
        epochs (int): number of passes over text corpa to pass to Doc2vec
    ...
    if print_sim:
        ...
        print('Vector similiarity: ',
              model.docvecs.similarity('2', '12'), '\n')

        print(it.doc_list[150], ', ', it.doc_list[154])
        print('Vector similiarity: ',
              model.docvecs.similarity('150', '154'), '\n')

        print(it.doc_list[4], ', ', it.doc_list[417])
        print('Vector similiarity: ',
              model.docvecs.similarity('4', '417'), '\n')

    return model


def TF_IDF(product_name, print_sim=True, stop_words='english',
           ngram_range=(1, 2), analyzer='word', min_df=1, max_features=None):
    """
    ...
    Args:
        product_name (Series): product names to be vectorised
        print_sim (Bool): If true vector cosine similaries will be printed
    ...
    Return:
        (tuple):
        :array: tfidf array
        :object: tfidf fitted model
    """
    # create tfidf object
    tf = TfidfVectorizer(stop_words=stop_words, ngram_range=ngram_range,
...
def fasttext_vectors(product_name, size=100, window=5, mincount=3, n_iter=5):
    """
    Function to calcuate fastText vectors for product names.
    see gensim documnetation
    Args:
        product_name (Series): product names to be vectorised
        size (int): length of the vectors
        window(int): window function size
        mincount (int): the minimum count frequency for a word to be included
    ...
    # train the model
    model_fasttext = FastText(product_name, size=size, window=window,
                              min_count=mincount,
                              sg=1, hs=1, iter=n_iter, negative=10)
...
def fasttext_infvec(product_name, model_fasttext):
    ...
    return:
        array: the infered word vectors for the products
    """
    product_name = product_name.str.split(' ')
...
def fit_word2vec(product_name, print_sim=True, vector_size=10, window=12,
                 min_count=1, workers=4, alpha=0.025,
                 min_alpha=0.001, epochs=100):
    """
    Function to calcuate word2vec vectors for product names.
    see gensim documnetation
    Args:
        product_name (Series): product names to be vectorised
        print_sim (Bool): If true vector cosine similaries will be printed
        ...
        alpha (float): learning rate for word2vec
    ...
    product_name = product_name.str.split(' ')
    # train model
    model_word2vec = Word2Vec(product_name, size=vector_size, window=window,
                              min_count=min_count, workers=workers,
                              min_alpha=min_alpha, iter=epochs)
...
def word2vec(product_name, model_word2vec):
    ...
    i = 0
    vectors = []
    # loop through the product names inferning the vectors in tern
    for nlist in product_name:
